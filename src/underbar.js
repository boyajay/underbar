(function() {
  ...
  _.last = function(array, n) {
        return n > array.length ? array.slice() :
          (n === undefined ? array[array.length-1] : array.slice(array.length-n,array.length));
  };
  ...
  _.uniq = function(array) {
    var passed = [];
    _.each(array, function(item){if (_.indexOf(passed, item) === -1) { passed.push(item);}});
  return passed;
  };
  ...
  _.map = function(collection, iterator) {
     ...
     var results = [];
    _.each(collection, function(item){ results.push(iterator(item));});
    return results;
